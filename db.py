...
2) MODO LOCAL (padrão para desenvolvimento na sua máquina):
   ...
   (financas.db), como antes.

IMPORTANTE (correção de estabilidade): cada operação de banco abre sua PRÓPRIA
conexão curta, executa, e fecha em seguida — em vez de reaproveitar uma única
conexão para várias instruções seguidas. Isso evita o erro conhecido do Turso
"Hrana: stream not found" (a conexão/stream expira no servidor quando fica
aberta por muito tempo executando várias instruções em sequência). Além disso,
se esse erro específico ocorrer mesmo assim, o sistema tenta novamente
automaticamente uma vez, com uma conexão totalmente nova.
"""
...
# ---------------------------------------------------------------------------
# Primitivas de execução: cada uma abre SUA PRÓPRIA conexão isolada e tenta
# novamente uma vez (com conexão nova) se detectar o erro de stream expirado.
# ---------------------------------------------------------------------------
...
def init_db():
    ...
    total_categorias = fetch_one("SELECT COUNT(*) AS n FROM categorias")["n"]
...
def add_categoria(nome: str, teto_mensal: float = 0.0):
    execute("INSERT INTO categorias (nome, teto_mensal) VALUES (?, ?)", (nome.strip(), teto_mensal))
...
def add_cartao(nome: str, dia_fechamento: int, dia_vencimento: int):
    execute("INSERT INTO cartoes (nome, dia_fechamento, dia_vencimento) VALUES (?, ?, ?)",
            (nome.strip(), dia_fechamento, dia_vencimento))


def list_cartoes():
    return fetch_all("SELECT * FROM cartoes ORDER BY nome")
...
def add_receita(data_str: str, origem: str, valor: float, observacao: str = ""):
    execute("INSERT INTO receitas (data, origem, valor, observacao) VALUES (?, ?, ?, ?)",
...
def gerar_despesas_fixas_do_mes(ano: int, mes: int):
    ...
    criados = 0
    fixas = fetch_all("SELECT * FROM despesas_fixas WHERE ativa = 1")
    for f in fixas:
        ja_existe = fetch_one("""
