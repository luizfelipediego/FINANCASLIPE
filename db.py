...
def _executar_migracao_uma_vez(nome: str, funcao):
    ...
    try:
        ...
        print(f"[MIGRAÇÃO] '{nome}' não concluída: {type(e).__name__}: {e}", file=sys.stderr)
...
def _reconciliar_residuos_de_tentativas_antigas(tabela: str, colunas: list):
    ...
    for nome_residuo in (f"{tabela}_migracao_old", f"{tabela}__nova_sem_unique"):
        ...
        try:
            ...
            print(f"[MIGRAÇÃO] Falha ao resgatar dados de '{nome_residuo}': {type(e).__name__}: {e}", file=sys.stderr)
        try:
            execute(f"DROP TABLE IF EXISTS {nome_residuo}")
        except Exception as e:
            print(f"[MIGRAÇÃO] Não foi possível apagar resíduo '{nome_residuo}': {type(e).__name__}: {e}", file=sys.stderr)
...
def _remover_unique_sem_recriar_tabela(tabela: str, coluna: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        print(f"[MIGRAÇÃO] Falha ao remover UNIQUE de '{tabela}.{coluna}': {type(e).__name__}: {e}", file=sys.stderr)
        return False
    finally:
        conn.close()
...
def delete_categoria(categoria_id: int, requesting_user: dict):
    ...
    if n_despesas or n_fixas:
        raise RegistroVinculadoError(
            ...
            f"{n_fixas} despesa(s) fixa(s) usando essa categoria."
        )
...
def delete_cartao(cartao_id: int, requesting_user: dict):
    ...
    if n_despesas or n_fixas:
        raise RegistroVinculadoError(
            f"Não é possível excluir '{before['nome']}': existem {n_despesas} despesa(s) e "
            f"{n_fixas} despesa(s) fixa(s) lançadas nesse cartão."
        )
...
def list_compras_por_tipo(requesting_user: dict = None, forma_pagamento: str = None,
    ...
    Ajustado para garantir que 'valor_parcela' esteja SEMPRE presente nos dicionários retornados.
    """
    if not requesting_user:
        return []
    ...
    for grupo, rows in grupos.items():
        ...
        valor_parcela = round(valor_total / parcela_total, 2) if parcela_total else valor_total
